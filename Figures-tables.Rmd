...
# Changing small values to white
cols[4]<-cols[6]<-"#F7F7F7"
...
#cols<-cols[3:14] # Asymmetric diverging palette
#show_col(cols)
colscale<-c(10^12,10^11,10^10,10^9,10^8,10^7,10^6,10^5,10^4,10^3,10^2,10,0,-10,-10^2,-10^3,-10^4,-10^5,-10^6,-10^7,-10^8,-10^9,-10^10,-10^11,-10^12)
labs<-expression(paste("10"^"12"),"",paste("10"^"10"),"",paste("10"^"8"),"",paste("10"^"6"),"",paste("10"^"4"),"",paste("10"^"2"),"",0,"",paste("-10"^"2"),"",paste("-10"^"4"),"",paste("-10"^"6"),"",paste("-10"^"8"),"",paste("-10"^"10"),"",paste("-10"^"12"))
#colscale<-unname(quantile(tresult.ni$net.damage, probs = seq(0, 1, 1/4)))
#colscale<-c(min(tresult$net.damage), min(tresult$net.damage)/2, min(tresult$net.damage)/4, min(tresult$net.damage)/8, 0, max(tresult$net.damage)/8, max(tresult$net.damage)/4, max(tresult$net.damage)/2, max(tresult$net.damage))
...
```{r, echo=FALSE}
# To use an asymmetric diverging palette, use a symmetric one and drop some from one or both sides https://www.r-bloggers.com/how-to-expand-color-palette-with-ggplot-and-rcolorbrewer/ 
getPalette<-colorRampPalette(brewer.pal(26, "RdBu")) # function, change if different palette is desired
cols<-getPalette(26) # Needs to be odd number to ensure midpoint color is correct
#cols<-cols[3:14] # Asymmetric diverging palette
#show_col(cols)
labs<-expression(paste("10"^"12"),"",paste("10"^"10"),"",paste("10"^"8"),"",paste("10"^"6"),"",paste("10"^"4"),"",paste("10"^"2"),"",0,"",paste("-10"^"2"),"",paste("-10"^"4"),"",paste("-10"^"6"),"",paste("-10"^"8"),"",paste("-10"^"10"),"",paste("-10"^"12"))
#colscale<-unname(quantile(tresult.ni$net.damage, probs = seq(0, 1, 1/4)))
...
# cols[6:12]<-cols[14:20]<-"#F7F7F7"

# Template https://stackoverflow.com/questions/36637545/how-to-create-legend-continuous-colourbar-based-on-specific-range-of-values-in/36718315

# Damage as a factor
tresult$net.damage_f<-cut(tresult$net.damage,
                      breaks = c(+Inf,10^12,10^11,10^10,10^9,10^8,10^7,10^6,10^5,10^4,10^3,10^2,10,0,-10,-10^2,-10^3,-10^4,-10^5,-10^6,-10^7,-10^8,-10^9,-10^10,-10^11,-10^12,-Inf))

# Below based on advice:
# https://stackoverflow.com/questions/61415597/ggplot-custom-category-legend-for-shapefile
# https://stackoverflow.com/questions/55167464/manual-fill-geom-tile-with-multiple-scales-by-group

# Horizontal legend oesn't work because of expressions in legend labels - they are always left justified and can't be placed over breaks between bins
...
ggplot(data = tresult,aes(x=scen.x, y=Name, fill=net.damage_f)) + 
  ...
  theme_minimal() +
  labs(x = "OLU Protection Scenario", y = "OLU") +
  theme(legend.position = "right", legend.key.size = unit(.2, 'cm'), legend.text=element_text(size=10)) +
  guides(fill=guide_legend(ncol = 1, label.position = "left", title.position = "top", title = "Net Change Damage ($)", label.vjust = -1.75, reverse = TRUE))

```

```{r, echo=FALSE}
# To use an asymmetric diverging palette, use a symmetric one and drop some from one or both sides https://www.r-bloggers.com/how-to-expand-color-palette-with-ggplot-and-rcolorbrewer/ 
getPalette<-colorRampPalette(brewer.pal(14, "RdBu")) # function, change if different palette is desired
cols<-getPalette(14) # Needs to be odd number to ensure midpoint color is correct
#cols<-cols[3:14] # Asymmetric diverging palette
#show_col(cols)
labs<-expression(paste("10"^"12"),paste("10"^"10"),paste("10"^"8"),paste("10"^"6"),paste("10"^"4"),paste("10"^"2"),"",paste("-10"^"2"),paste("-10"^"4"),paste("-10"^"6"),paste("-10"^"8"),paste("-10"^"10"),paste("-10"^"12"))
...
# Damage as a factor
tresult$net.damage_f<-cut(tresult$net.damage,
                      breaks = c(+Inf,10^12,10^10,10^8,10^6,10^4,10^2,0,-10^2,-10^4,-10^6,-10^8,-10^10,-10^12,-Inf))
...
ggplot(data = tresult,aes(x=scen.x, y=Name, fill=net.damage_f)) + 
  ...
  guides(fill=guide_legend(ncol = 1, label.position = "left", title.position = "top", title = "Net Change Damage ($)", label.vjust = 2.25, reverse = TRUE))
...
tresult$net.damage_f<-cut(tresult$net.damage,
                      breaks = c(+Inf,10^12,10^10,10^8,10^6,10^4,10^2,0,-10^2,-10^4,-10^6,-10^8,-10^10,-10^12,-Inf))
tresult$net.damage_f<-tresult$net.damage_f
...
# Change small values to white
cols[3:4]<-cols[6:7]<-"#F7F7F7"
# Template https://stackoverflow.com/questions/36637545/how-to-create-legend-continuous-colourbar-based-on-specific-range-of-values-in/36718315
ggplot(data = fresult, aes(x=scen.x, y=OLU, fill=net.volume)) + 
  geom_tile() + 
  scale_fill_gradientn(colours = cols, values = rescale(colscale), breaks = colscale, labels = labs, name="Net Change Volume (Cubic Meters)", guide=guide_legend(), limits = c(-10^10,10^8)) +
...
# Wide to long on SLR
colnames(stst)<-c("Name","OLUid","idmg.050","idmg.100","idmg.150","idmg.200","edmg.050","edmg.100","edmg.150","edmg.200","ndmg.050","ndmg.100","ndmg.150","ndmg.200","Geomorphic","geom","area")
stst<-pivot_longer(stst,cols = ends_with(c("050","100","150","200")), names_to = c("Type", "SLR"), names_sep = "\\.", values_to = "Damage")
stst$Type<-ifelse(stst$Type=="idmg","internal", ifelse(stst$Type=="edmg","external","net"))
...
colnames(bf)<-c("Name","numCB","sumtrv","OLUid","iarea.100","iarea.150","iarea.200", "iarea.050","ivol.100","ivol.150","ivol.200","ivol.050","earea.100","earea.150","earea.200", "earea.050","evol.100","evol.150","evol.200","evol.050","Geomorphic","geom","area")

bf$nvol.050<-bf$ivol.050+bf$evol.050
bf$nvol.100<-bf$ivol.100+bf$evol.100
bf$nvol.150<-bf$ivol.150+bf$evol.150
bf$nvol.200<-bf$ivol.200+bf$evol.200

bf<-subset(bf, select = -c(iarea.100,iarea.150,iarea.200,iarea.050,earea.100,earea.150,earea.200,earea.050))
bf$Name<-NULL

bf<-pivot_longer(bf,cols = ends_with(c("050","100","150","200")), names_to = c("Type", "SLR"), names_sep = "\\.", values_to = "Volume")
bf$Type<-ifelse(bf$Type=="ivol","internal", ifelse(bf$Type=="evol","external","net"))

bf<-subset(bf, select = c(OLUid, Type, SLR, Volume))

# Joining damage and flood data
stst<-left_join(stst,bf,by=c("OLUid", "Type", "SLR"))
...
stst$SLR<-ifelse(stst$SLR=="050","50cm",
...
bf.e_comp<-ggplot(bf.e, aes(x=as.factor(SLR),y=net.volume,fill=as.factor(SLR))) +
  geom_boxplot(outlier.shape = NA) +
  theme_minimal() +
  theme(axis.text.x=element_blank(), axis.title.x = element_blank(), legend.title = element_blank(), legend.box = "horizontal")
...
```{r}
ggplot(fresult.ni, aes(x=as.factor(SLR),y=net.volume,fill=as.factor(SLR))) +
  geom_boxplot(outlier.shape = NA) +
  theme_minimal() +
  coord_cartesian(ylim=c(-250000, 750000)) +
  theme(axis.text.x=element_blank(), axis.title.x = element_blank(), legend.title = element_blank(), legend.box = "horizontal")
```
