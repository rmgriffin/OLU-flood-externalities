...
coast<-st_read("./Data/Shoreline.gpkg", crs = 4326)
wpop<-raster("./Data/ppp_2020_1km_Aggregated.tif")
...
# # Make sure layers are as expected
# tm_shape(wpop) +
#   tm_raster() +
#   tm_shape(coast) +
#   tm_lines(col = "red")
...
# Function for percent population in class 2, as a function of coastline buffer size

CBP<-function(b){

  cst<-st_buffer(coast,b)

  cst$sumpop<-exact_extract(wpop, cst, "sum")
  a<-sum(cst[cst$FIN_TYP==2,]$sumpop)/sum(cst$sumpop)*100
  e<-sum(cst[cst$FIN_TYP==2,]$sumpop)/1000000
  f<-sum(cst$sumpop)/1000000
...
tz<-st_buffer(coast,100000)
  tm_shape(tz) +
  tm_polygons(border.col = "red")
  
# When you buffer discrete line segments, they overlap with buffers from other segments. Solution to this is to create two separate layers, one where all shoreline segments are buffered, then removing internal boundaries using dissolve, and another where only class 2 segments are buffered and dissolved. The first estimates total shoreline population within x distance, the second gets you population within x distance for class 2. In both cases, dissolving makes sure no double counting takes place.  
```